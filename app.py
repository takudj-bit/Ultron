...
企画書に含まれる固有名詞（アーティスト名・作品名・ブランド名など）を正確に読み取り、それぞれについて具体的に調査してください。

【1. メインアーティスト/プロジェクトのポジション分析】
- 企画書に登場するアーティストの現在の市場ポジション
- ストリーミング数、最近のリリースの評価、チャート実績
- 強み（他にない武器）と弱み（ファンや批評家からの指摘）
- キャリアの現在地：上昇期/安定期/再起期のどこか
- 競合・同ジャンルのアーティストとの差別化ポイント

【2. 関連IP・タイアップ先・コンテキストの分析】
- 企画書にタイアップ先（アニメ、映画、ゲーム、ブランド等）があれば、そのファン層を調査
  - 年齢・性別・地域の分布
  - ファンの感情的な反応傾向（何に共感し、何で盛り上がるか）
  - SNSでの行動パターン（二次創作、ミーム、推し活）
  - 作品/ブランドのテーマ・世界観のコア要素
- タイアップがない場合は、企画書のターゲット層を分析

【3. 類似成功事例の分析】
- 企画書の内容に近いコンセプト・ジャンル・用途の楽曲でヒットした事例
- それらの共通点: BPM、キー、歌詞テーマ、曲構成、音楽的特徴
- ヒットの要因分析（作品との相性、バイラル要因、タイミング）
- 失敗事例とその原因

【4. ターゲットリスナーのトレンド分析】
- 企画書から読み取れるターゲット層に今刺さっている音楽の特徴
- 関連するジャンルミックスやサウンドの成功事例
- SNS（TikTok/Reels/YouTube等）でバイラルする音楽の共通要素
- 今後6ヶ月のトレンド予測

【5. コラボレーション・追加要素の可能性】
- 企画書にフィーチャリング/コラボの言及があれば、最適な候補の傾向を調査
  - 声質、ジャンル、ファン層の相性
  - 過去の類似コラボの成功/失敗パターン
- コラボの言及がなくても、プロジェクトを強化できる追加要素（プロデューサー、サウンドの方向性等）を提案

ルール:
- 企画書に書かれている固有名詞はすべて調査対象にする
- 具体的な曲名・アーティスト名を挙げて根拠を示す
- 数字・データ（再生数、チャート順位、フォロワー数など）を可能な限り含める
- 企画書に書かれていない軸は「該当情報なし」と明記してスキップ可
- 日本語で回答
- 各セクションを見出しで明確に分ける
- 推測と事実を区別する
"""
            ...
            response = perplexity_client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": RESEARCH_SYSTEM_PROMPT},
                    {
                        "role": "user",
                        "content": (
                            "以下のクライアント企画書を分析してください。\n"
                            "企画書内のアーティスト名・作品名・ジャンル・用途をすべて読み取り、"
                            "それぞれについて5軸で調査してください。\n\n"
                            f"{brief}"
                        ),
                    },
